...
for sec in data.get('sections', []):
    for el in sec.get('elements', []):
        for conc in el.get('subConcepts', []):
            if conc.get('type') == 'list':
                questions.append({
                    'text': conc.get('title', ''),
                    'options': conc.get('items', [])
                })
...
for q in questions:
    ...
    if correct_ans:
        ...
        entries.append(f"  {{ question: '{q_safe}', correctAnswer: '{correct_ans}', source: 'docs/assessments/اجابة الاختبار.docx', rationale: 'مستخرج من ملف الإجابة.' }},")
    else:
